...
> **研究主线的完整方案（研究问题 / 假设 / 算法定义 / 评测协议）见 [research/ADAPTIVE_CODE_RAG.md](./research/ADAPTIVE_CODE_RAG.md)。**

## 一句话

> 我不是在复刻 Codex，而是在研究如何把 Coding Agent 变成一个**能理解代码、受权限控制、在隔离环境执行、由确定性测试验证、并能通过真实轨迹持续评测和改进**的可靠执行系统。

> 研究主线（下一阶段）：**不研究"怎么把更多代码找回来"，而研究"Coding Agent 如何以最少的检索成本，主动获取足够且可靠的代码证据"。**
