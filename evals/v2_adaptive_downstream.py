...
def evaluate(replay,tasks_path):
 ...
 with tempfile.TemporaryDirectory(prefix="v2-adaptive-oracle-") as tmp:
  base=Path(tmp)
  for rr in replay["rows"]:
   sid=source_problem_id(rr["task_id"]);spec=specs[sid];root=base/sid;prepare(spec,root)
   allowed={x["path"] for x in rr["adaptive"]["evidence_payload"]};written=[]
   for rel,c in spec.gold_sources.items():
    if rel in allowed:(root/rel).write_text(c,encoding="utf-8");written.append(rel)
   g=grade(spec,root);rows.append({"task_id":rr["task_id"],"resolved":g["resolved"],"gold_written":written,"allowed_files":sorted(allowed)})
 return {"protocol":"v2-adaptive-oracle-downstream-v1","tasks":len(rows),"resolved":sum(r["resolved"] for r in rows),"rows":rows,
 "claim_boundary":"Constrained Oracle upper-bound only; not autonomous repair success."}
def main():
