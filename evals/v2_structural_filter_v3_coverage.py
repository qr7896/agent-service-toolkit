...
def score(replay,tasks_path):
 ...
 return {"protocol":"v2-structural-filter-v3-coverage-v1","escalated_tasks":len(rows),"gold_hit_tasks":sum(x["gold_hit"] for x in rows),"all_gold_hit":all(x["gold_hit"] for x in rows),"rows":rows,"claim_boundary":"Gold retrospective only."}
def main():
