...
def score(replay,tasks_path):
 ...
 for r in replay["rows"]:
  ...
  kept=select(r["adaptive"]["evidence_payload"]);gold=set(specs[source_problem_id(r["task_id"])].gold_files);hits={x["path"] for x in kept}&gold
  rows.append({"task_id":r["task_id"],"kept":len(kept),"gold_hit":bool(hits),"gold_paths_hit":sorted(hits)})
 return {"protocol":"v2-structural-filter-v2-coverage-v1","escalated_tasks":len(rows),"gold_hit_tasks":sum(x["gold_hit"] for x in rows),"all_gold_hit":all(x["gold_hit"] for x in rows),"rows":rows,
 "claim_boundary":"Gold is retrospective scoring only."}
def main():
